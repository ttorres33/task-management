...
def md_files(directory):
    ...
    Case-insensitive, with a codepoint tiebreaker to keep the order total. The shell
    glob this replaced sorted by LC_COLLATE, and a real Claude Code session collates
    case-insensitively. Established from evidence rather than reasoning: a today.md
    generated by the previous version listed "What changes" *after* "teresabot", which
    only a case-insensitive comparison produces. Plain sorted() puts every capitalized
    filename first, which visibly reshuffles the daily file for any vault with titled
    task names -- and most real vaults have some.

    Beware measuring this from a tool-invoked shell: those can run with no locale set
    at all, where collation falls back to C and sorted() looks correct. That is not
    the environment the plugin runs in.
